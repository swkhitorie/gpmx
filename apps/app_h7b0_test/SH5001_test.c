                                ...
                                unsigned char *readBuf)
{
    int ret;
    uint8_t cmdad;
    cmdad =  (uint8_t)(regAddr);

    struct i2c_msg_s rxmsg;
    rxmsg.flags = I2C_REG_READ;
    rxmsg.addr = devAddr<<1; rxmsg.reg_sz = 1;
    rxmsg.xbuffer = &cmdad;
    rxmsg.xlength = 1;
    rxmsg.rbuffer = readBuf;
    rxmsg.rlength = readLen;
    ret = I2C_TRANSFER(test_ops, &rxmsg, 1);
                                ...
                                unsigned char *writeBuf)
{
    ...
    wmsg.reg_sz = 1;
    wmsg.xbuffer = &cmd[0];
    wmsg.xlength = writeLen+1;
    ret = I2C_TRANSFER(test_ops, &wmsg, 1);
	return (SH5001_TRUE);
}																
...
*               2.accCutOffRreq = accODR * 0.40 or accODR * 0.24 or accODR * 0.12 or accODR * 0.04;
*
* Parameters: 	accODR              	accRange                accCutOffFreq       	accFilter				accByPass
*               SH5001_ACC_ODR_880HZ	SH5001_ACC_RANGE_16G	SH5001_ACC_ODRX040		SH5001_ACC_FILTER_EN	SH5001_ACC_BYPASS_EN
*               SH5001_ACC_ODR_440HZ	SH5001_ACC_RANGE_8G		SH5001_ACC_ODRX024		SH5001_ACC_FILTER_DIS	SH5001_ACC_BYPASS_DIS
*               SH5001_ACC_ODR_220HZ	SH5001_ACC_RANGE_4G		SH5001_ACC_ODRX012
*               SH5001_ACC_ODR_110HZ	SH5001_ACC_RANGE_2G		SH5001_ACC_ODRX004
*  
* return:	void
*
******************************************************************/
unsigned char storeAccODR;
static void SH5001_Acc_Config(unsigned char accODR, 
                              unsigned char accRange, 
                              unsigned char accCutOffFreq,
                              unsigned char accFilter,
...
*               SH5001_GYRO_ODR_880HZ	SH5001_GYRO_RANGE_125	SH5001_GYRO_ODRX040	SH5001_GYRO_FILTER_EN	SH5001_GYRO_BYPASS_EN
*               SH5001_GYRO_ODR_440HZ	SH5001_GYRO_RANGE_250	SH5001_GYRO_ODRX024	SH5001_GYRO_FILTER_DIS	SH5001_GYRO_BYPASS_DIS
*               SH5001_GYRO_ODR_220HZ	SH5001_GYRO_RANGE_500	SH5001_GYRO_ODRX012
*               SH5001_GYRO_ODR_110HZ	SH5001_GYRO_RANGE_1000  SH5001_GYRO_ODRX004
*
* return:	void
*
******************************************************************/
...
unsigned char SH5001_SwitchPowerMode(unsigned char powerMode)
{
	...
	unsigned char reg_addr[SH5001_POWMODE_REG_NUM] = {0x20, 0x30, 0x2B, 0xBD, 0xD1, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8, 0xD9, 0x69, 0x74, 0x80};
	unsigned char reg_data_normal[SH5001_POWMODE_REG_NUM] = {0x27, 0x08, 0x00, 0x0C, 0x68, 0x00, 0x00, 0x01, 0x50, 0x00, 0x99, 0x0C, 0x0C, 0x0C};
	unsigned char reg_data_acc_hp[SH5001_POWMODE_REG_NUM] = {0x27, 0x08, 0x01, 0x2C, 0x6B, 0x00, 0x7A, 0xFF, 0xFC, 0x00, 0x98, 0x08, 0x08, 0x08};
	unsigned char reg_data_acc_lp[SH5001_POWMODE_REG_NUM] = {0xA7, 0x08, 0x01, 0x2C, 0x6B, 0x90, 0x7A, 0xFF, 0xFC, 0x00, 0x98, 0x04, 0x04, 0x04};
	unsigned char reg_data_powerdown[SH5001_POWMODE_REG_NUM] = {0xA5, 0x08, 0x01, 0x0E, 0x6B, 0xF1, 0xFE, 0xFF, 0xFC, 0xE2, 0x98, 0x0C, 0x0C, 0x0C};
	int CodeOld;
	int CodeNew;
    short RatioOld;
    short RatioNew;
	...
	case SH5001_NORMAL_MODE:
		...
		for(i = 0; i < SH5001_POWMODE_REG_NUM; i++)
		{
...
void SH5001_GetImuData( short accData[3], short gyroData[3] )
{
	...
	float accel_g[3];
	for (int i = 0; i < 3; i++) {
		gyro_deg[i] = accData[i] * (float)((2 * 16) / 65536.0);
		accel_g[i] =  gyroData[i] * (float)((2 * 2000.0) / 65536.0);
	}

	//printf("%.3f %.3f %.3f %.3f %.3f %.3f\r\n", accel_g[0], accel_g[1], accel_g[2], gyro_deg[0], gyro_deg[1], gyro_deg[2]);
	printf("%d %d %d %d %d %d\r\n", accData[0], accData[1], accData[2], gyroData[0], gyroData[1], gyroData[2]);
}
...
void sh5001_gyro_trim_quad(unsigned char quadval[6])
{
  ...
  for (i = 0; i < 3; i++)
  {
      ...
      RegWrt_1[i + 7] = (quadData[i + 7] & 0xF8) + S_Gyro_Quad[i] * 4 + ((C_Gyro_Quad[0] & 0x0300) >> 8);
