...
bool prb_is_full(struct __prbuf *obj) { return (obj->size == RBUFFER_LENGTH); }
...
uint16_t prb_write(struct __prbuf *obj, const uint8_t *p, uint16_t len)
{
	...
	rssize = RBUFFER_LENGTH - obj->size;
	if (rssize >= len) {
		wlen = len;
	} else {
        wlen = rssize;
	...
	for (i = 0; i < wlen; i++) {
		obj->buf[obj->in] = p[i];
		obj->in++;
		if (obj->in >= RBUFFER_LENGTH) {
			obj->in -= RBUFFER_LENGTH;
		}
        obj->size++;
	}
...
uint16_t prb_read(struct __prbuf *obj, uint8_t *p, uint16_t len)
{
	...
	for (i = 0; i < rlen; i++) {
		p[i] = obj->buf[obj->out];
		obj->out++;
		if (obj->out >= RBUFFER_LENGTH) {
			obj->out -= RBUFFER_LENGTH;
		}
        obj->size--;
	}
