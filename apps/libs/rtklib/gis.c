...
*-----------------------------------------------------------------------------*/
#include "rtklib.h"
#if defined(CONFIG_RTKLIB_GIS_DATA)
...
static int gis_add(gisd_t **p, int type, void *data)
{
    ...
    if (!(new_data=(gisd_t *)rtkmalloc(sizeof(gisd_t)))) {
        return 0;
    }
    new_data->next=*p;
    new_data->type=type;
...
static int read_pnt(FILE *fp, double *bound, gisd_t **p)
{
    ...
    if (!(pnt=(gis_pnt_t *)rtkmalloc(sizeof(gis_pnt_t)))) {
        return 0;
    }
...
static int read_poly(FILE *fp, double *bound, gisd_t **p)
{
    ...
    if (!(part=(int *)rtkmalloc(sizeof(int)*nt))) {
        return 0;
    }
    for (i=0;i<nt;i++) {
        fread(buff,4,1,fp);
    ...
    for (i=0;i<nt;i++) {
        ...
        if (!(poly=(gis_poly_t *)rtkmalloc(sizeof(gis_poly_t)))) {
            rtkfree(part);
            return 0;
        }
        if (!(poly->pos=(double *)rtkmalloc(sizeof(double)*nr*3))) {
            rtkfree(poly);
            rtkfree(part);
            return 0;
        }
...
static int read_polygon(FILE *fp, double *bound, gisd_t **p)
{
    ...
    for (i=0;i<nt;i++) {
        ...
        if (!(polygon=(gis_polygon_t *)rtkmalloc(sizeof(gis_poly_t)))) {
            rtkfree(part);
            return 0;
        }
        if (!(polygon->pos=(double *)rtkmalloc(sizeof(double)*nr*3))) {
            rtkfree(polygon);
            rtkfree(part);
            return 0;
        }
...
/* rtkfree gis-data ---------------------------------------------------------------
* rtkfree and initialize gis data
* args   : gis_t  *gis      IO  gis data
* return : none
*-----------------------------------------------------------------------------*/
extern void gis_rtkfree(gis_t *gis)
{
    gisd_t *data,*next;
    int i;
    ...
    for (i=0;i<MAXGISLAYER;i++) {
        for (data=gis->data[i];data;data=next) {
            next=data->next;
            if (data->type==2) {
                rtkfree(((gis_poly_t *)data->data)->pos);
            }
            else if (data->type==3) {
                rtkfree(((gis_polygon_t *)data->data)->pos);
            }
            rtkfree(data);
        }
        gis->data[i]=NULL;
...
#endif  // end with defined(CONFIG_RTKLIB_GIS_DATA)
