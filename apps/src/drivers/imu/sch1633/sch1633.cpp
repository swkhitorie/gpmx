...
int SCH1_setFilters(uint32_t Freq_Rate12, uint32_t Freq_Acc12, uint32_t Freq_Acc3)
{
    ...
    if (((requestFrame_Rate12 & TA_FIELD_MASK) >> 38) != ((responseFrame_Rate12 & SA_FIELD_MASK) >> 37)) {
        printf("che1: %llx \r\n ", ((requestFrame_Rate12 & TA_FIELD_MASK) >> 38));
        printf("che2: %llx \r\n ", ((responseFrame_Rate12 & SA_FIELD_MASK) >> 37));
...
{
	uint64_t val = hw_SPI48_Send_Request(Request);
    return val;
}
...
int SCH1_init(SCH1_filter sFilter, SCH1_sensitivity sSensitivity, SCH1_decimation sDecimation, bool enableDRY)
{
    ...
    for (startup_attempt = 0; startup_attempt < 2; startup_attempt++) {
        ...
        // Set user controls
        int a1 = SCH1_setFilters(sFilter.Rate12, sFilter.Acc12, sFilter.Acc3);
        int a2 = SCH1_setRateSensDec(sSensitivity.Rate1, sSensitivity.Rate2, sDecimation.Rate2);
        int a3 = SCH1_setAccSensDec(sSensitivity.Acc1, sSensitivity.Acc2, sSensitivity.Acc3, sDecimation.Acc2);
        int a4;
        if (enableDRY)
            a4 = SCH1_setDRY(0, true);   // 0 = DRY active high
        else
            a4 = SCH1_setDRY(0, false);

        // Write EN_SENSOR = 1
        int a5 = SCH1_enableMeas(true, false);
