...
void p2p_raw_ackfhss_sender_process(struct __p2p_obj *obj)
{
    ...
    case 0x11: {
            ...
            p2p_dw_channel_next(obj, obj->_dw_ch_rssi, obj->_dw_ch_snr);
            ...
            connect_dataheader.dw_fq_idx = obj->_ch_grp.dw_fq_idx;
    ...
    case 0x12: {
            if (p2p_is_tx_done(obj)) {
                ...
                p2p_set_channel(obj, obj->_ch_grp.up_list[obj->_ch_grp.up_fq_idx_fix].freq);
    ...
    case 0x13: {
            ...
            if (P2P_ELAPSED_TIME(obj->_ack_fail_timestamp) > P2P_ACK_TIMEOUT) {
                ...
                if (obj->_ack_fail_retry_cnter > P2P_ACK_TIMEOUT_MAX_TRY_TIMES) {
                    ...
                    p2p_dw_channel_next(obj, -100, -10);
