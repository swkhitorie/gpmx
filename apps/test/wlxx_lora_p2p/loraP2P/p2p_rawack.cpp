 ...
 * Sender 
 ****************************************************************************/
void lora_p2p_raw_ack_sender_process(lora_state_t *obj)
{
    uint8_t c;
    size_t rsz, rsz2;
    int ret;
            ...
            if (rsz > 0) {
                ...
                case 0: {
                        ...
                        if (ret == 0 && con_ret.snd_auth_key == obj->auth_key_board) {
                            ...
                            LORAP2P_DEBUG("Ack tick:%d, rssi:%d, snr:%d, total snd:%d, total acktime: %d\r\n",
                                obj->tick, obj->rssi, obj->snr, obj->msg_nbytes_total_snd,
                                obj->pack_ack_completed_time);
 ...
 * Receiver 
 ****************************************************************************/
void lora_p2p_raw_ack_receiver_process(lora_state_t *obj)
{
    uint8_t c;
    size_t rsz;
    int ret;
    ...
    case 0x11: {
            ...
            if (board_elapsed_tick(obj->link_failed_timestamp) > LORAP2P_LINK_FAIL_TIMEOUT) {
                // re-enter
                lora_p2p_state_to_linkfind(obj);
            }
...
void lora_p2p_raw_ack_process(lora_state_t *obj)
{
    if (obj->role == LORA_SENDER) {
        lora_p2p_raw_ack_sender_process(obj);
    } else if (obj->role == LORA_RECEIVER) {
        lora_p2p_raw_ack_receiver_process(obj);
    }
}
