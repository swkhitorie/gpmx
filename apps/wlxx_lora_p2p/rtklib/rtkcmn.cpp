...
{
    uint8_t cnt = 0;
    cnt = bits_table[value&0xff] + bits_table[(value>>8)&0xff];
    return cnt;
}
...
int input_rtcm3(rtcm_t *rtcm, unsigned char data)
{
    ...
    if (rtcm->nbyte<3||rtcm->nbyte<rtcm->len+3) return -2;
    rtcm->nbyte=0;
