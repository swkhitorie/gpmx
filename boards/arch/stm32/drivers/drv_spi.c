...
static uint32_t up_spi_send(struct spi_dev_s *dev, uint32_t wd);
...
uint32_t up_spi_send(struct spi_dev_s *dev, uint32_t wd)
{
    struct up_spi_dev_s *priv = dev->priv;
    uint16_t rd_word = 0;
    uint16_t wd_word = (wd & 0xffff);
    uint8_t transmit_len = dev->nbits / 8;

    if (HAL_OK != HAL_SPI_TransmitReceive(&priv->hspi, (uint8_t *)&wd_word, (uint8_t *)&rd_word, transmit_len, 1000)) {
        return -1;
    }
