...
#define W25QXX_WAIT_TIMESTAMP          board_get_time

#if defined(CONFIG_LFS_ENABLE)
lfs_t *lfs_array[5];
void lfs_objset(lfs_t *obj, uint8_t num) { lfs_array[num] = obj; }
lfs_t *lfs_objget(uint8_t num) { return lfs_array[num]; }
...
static lfs_t lfs_spiflash_w25q;
...
int w25qxx_init(int nbus, bool lfs_enable)
{
    int ret = 0, ret2 = 0;
    uint32_t id = 0;
    struct __w25q_qh *handle = w25qxx_handle_get();
    ...
    if (lfs_enable) {
#if defined(CONFIG_LFS_ENABLE)
        ret = lfs_mount(&lfs_spiflash_w25q, &w25qxx_lfs_cfg);
        if (ret != LFS_ERR_OK)  {
            ret = lfs_format(&lfs_spiflash_w25q, &w25qxx_lfs_cfg);
            ret2 = lfs_mount(&lfs_spiflash_w25q, &w25qxx_lfs_cfg);
            if (ret != LFS_ERR_OK || ret2 != LFS_ERR_OK) {
                return -3;
            }
        }

        lfs_objset(&lfs_spiflash_w25q, 0);
#endif
    }
            ...
            lfs_off_t off, void *buffer, lfs_size_t size)
{
    ...
    if (ret != 0) {
        BOARD_PRINTF("error w25qxx_lfs_read %d %x \r\n", ret, (uint32_t *)buffer);
        return LFS_ERR_IO;
    }
    return LFS_ERR_OK;
}
            ...
            lfs_off_t off, const void *buffer, lfs_size_t size)
{
    ...
    if (ret != 0) {
        BOARD_PRINTF("error w25qxx_lfs_prog %d\r\n", ret);
        return LFS_ERR_IO;
    }
    return LFS_ERR_OK;
}
...
int w25qxx_lfs_erase(const struct lfs_config *c, lfs_block_t block)
{
    ...
    if (ret != 0) {
        BOARD_PRINTF("error lfs_erase %d \r\n", ret);
        return LFS_ERR_IO;
    }
    return LFS_ERR_OK;
}
