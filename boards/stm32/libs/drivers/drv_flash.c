...
static void low_flash_unlock()
{
	HAL_FLASH_Unlock();
}

static void low_flash_lock()
{
	HAL_FLASH_Lock();
}

static uint8_t low_flash_get_sector(uint32_t addr)
{
...
static uint8_t low_flash_get_status(uint8_t bank_x)
{
	uint32_t status_reg;
...
static uint8_t low_flash_wait_handledone(uint8_t bank_x, uint32_t time)
{
	...
	res = low_flash_get_status(bank_x);
	/* clear error flag */
	if (res) {
		if (bank_x == 1)
...
static uint8_t low_flash_erase_sector(uint8_t sector_num)
{
	static int rescnt = 0;
	uint8_t res = 0;
	...
	while (res && rescnt < 10000) {
		res = low_flash_wait_handledone(sector_num / 8, 2000);
		rescnt++;
	}
	if (rescnt == 10000)
		return 0x11;
...
static uint32_t low_flash_read_word(uint32_t faddr)
{
...
static uint8_t low_flash_write_word(uint32_t faddr, uint32_t *pdata)
{
	...
	res = low_flash_wait_handledone(bank_x, 0XFF);	
...
uint8_t low_progflash_write(uint32_t WriteAddr, const uint32_t *pBuffer, uint32_t NumToWrite)
{
	uint8_t res = 0;
	uint8_t status = 0;
	uint32_t addrx = 0;
	...
	if (addrx < 0X1FF00000) {
		while (addrx < endaddr) {
			if (low_flash_read_word(addrx) != 0XFFFFFFFF) {   
				status = low_flash_erase_sector(low_flash_get_sector(addrx));
				if (status) {
					res = 0xFF;
					break;
				}
	...
	if (status == 0) {
		while (WriteAddr < endaddr) {
			if (low_flash_write_word(WriteAddr,pBuffer)) {
				res = 0xFF;
				break;
			}
			WriteAddr += 32;
...
void low_progflash_read(uint32_t ReadAddr, uint32_t *pBuffer, uint32_t NumToRead)
{
	uint32_t i;
