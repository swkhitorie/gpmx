...
static void ramtron_lock(struct ramtron_dev_s *priv);
...
static int ramtron_erase(struct mtd_dev_s *dev,
                        off_t startblock,
                        size_t nblocks);
static ssize_t ramtron_bread(struct mtd_dev_s *dev,
                            off_t startblock,
                            size_t nblocks, uint8_t *buf);

#ifdef CONFIG_RAMTRON_CHUNKING
static ssize_t ramtron_bwrite_nonchunked(struct mtd_dev_s *dev,
                                        off_t startblock,
                                        size_t nblocks,
                                        const uint8_t *buffer);
static ssize_t ramtron_bwrite_chunked(struct mtd_dev_s *dev,
                                    off_t startblock,
                                    size_t nblocks,
                                    const uint8_t *buf);
#endif
static ssize_t ramtron_bwrite(struct mtd_dev_s *dev,
                            off_t startblock,
                            size_t nblocks,
                            const uint8_t *buf);
static ssize_t ramtron_read(struct mtd_dev_s *dev,
                            off_t offset,
                            size_t nbytes,
                            uint8_t *buffer);
static int ramtron_ioctl(struct mtd_dev_s *dev,
                        int cmd,
                        unsigned long arg);
...
static inline int ramtron_pagewrite(struct ramtron_dev_s *priv,
                                    const uint8_t *buffer, off_t page,
                                    size_t pagesize)
{
...
static ssize_t ramtron_bread(struct mtd_dev_s *dev, off_t startblock,
                             size_t nblocks, uint8_t *buffer)
{
    struct ramtron_dev_s *priv = (struct ramtron_dev_s *)dev;
    ssize_t nbytes;
...
{
    struct ramtron_dev_s *priv = (struct ramtron_dev_s *)dev;
    const struct ramtron_parts_s *part;
    size_t blocksleft = nblocks;
    uint32_t p;
...
#ifdef CONFIG_RAMTRON_CHUNKING
static ssize_t ramtron_bwrite(struct mtd_dev_s *dev, off_t startblock,
