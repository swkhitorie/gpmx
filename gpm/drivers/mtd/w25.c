...
static void w25_lock(struct spi_dev_s *spi);
static inline void w25_unlock(struct spi_dev_s *spi);
static inline int w25_readid(struct w25_dev_s *priv);
#ifndef CONFIG_W25_READONLY
static void w25_unprotect(struct w25_dev_s *priv);
#endif
...
static bool w25_is_erased(struct w25_dev_s *priv,
                        off_t address,
                        off_t size);
static void w25_sectorerase(struct w25_dev_s *priv,
                            off_t offset);
static inline int w25_chiperase(struct w25_dev_s *priv);
static void w25_byteread(struct w25_dev_s *priv,
                         uint8_t *buffer,
                         off_t address,
                         size_t nbytes);
#ifndef CONFIG_W25_READONLY
static void w25_pagewrite(struct w25_dev_s *priv,
...
static uint8_t *w25_cacheread(struct w25_dev_s *priv,
                            off_t sector);
static void w25_cacheerase(struct w25_dev_s *priv,
                            off_t sector);
static void w25_cachewrite(struct w25_dev_s *priv,
                            const uint8_t *buffer,
...
static int w25_erase(struct mtd_dev_s *dev,
                    off_t startblock,
                    size_t nblocks);
static ssize_t w25_bread(struct mtd_dev_s *dev,
                        off_t startblock,
                        size_t nblocks,
                        uint8_t *buf);
static ssize_t w25_bwrite(struct mtd_dev_s *dev,
                        off_t startblock,
                        size_t nblocks,
                        const uint8_t *buf);
static ssize_t w25_read(struct mtd_dev_s *dev,
                        off_t offset,
                        size_t nbytes,
                        uint8_t *buffer);
static int w25_ioctl(struct mtd_dev_s *dev,
                    int cmd,
                    unsigned long arg);
#if defined(CONFIG_MTD_BYTE_WRITE) && !defined(CONFIG_W25_READONLY)
static ssize_t w25_write(struct mtd_dev_s *dev,
                        off_t offset,
                        size_t nbytes,
                        const uint8_t *buffer);
#endif
...
static void w25_byteread(struct w25_dev_s *priv, uint8_t *buffer,
...
#ifndef CONFIG_W25_READONLY
static void w25_pagewrite(struct w25_dev_s *priv, const uint8_t *buffer,
                        off_t address, size_t nbytes)
{
    uint8_t status;
...
#if defined(CONFIG_W25_SECTOR512) && !defined(CONFIG_W25_READONLY)
static uint8_t *w25_cacheread(struct w25_dev_s *priv, off_t sector)
{
    off_t esectno;
    int   shift;
    int   index;
                    ...
                    size_t nblocks)
{
    ...
    struct w25_dev_s *priv = (struct w25_dev_s *)dev;
...
static ssize_t w25_bwrite(struct mtd_dev_s *dev, off_t startblock,
...
static int w25_ioctl(struct mtd_dev_s *dev, int cmd, unsigned long arg)
{
    struct w25_dev_s *priv = (struct w25_dev_s *)dev;
...
struct mtd_dev_s *w25_initialize(struct spi_dev_s *spi)
{
    struct w25_dev_s *priv;
    int ret;
    ...
    */
    ...
    priv = (struct w25_dev_s *)kmm_zalloc(sizeof(struct w25_dev_s));
        ...
        */
        ...
        if (ret != 0) {
            ...
            */
            ...
            priv->sector = (uint8_t *)kmm_malloc(W25_SECTOR_SIZE);
