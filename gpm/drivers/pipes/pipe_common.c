...
static int pipecommon_semtake(sem_t *sem)
{
...
int pipecommon_open(struct file *filep)
{
    ...
    * sem_wait() call should fail if we are awakened by a signal or if the
    * thread was canceled.
    */
    ...
    */
    ...
    if (inode->i_crefs == 1 && dev->d_buffer == NULL) {
        ...
        dev->d_buffer = (uint8_t *)kmm_malloc(dev->d_bufsize);
        if (!dev->d_buffer) {
            ...
            dev->d_wrndx == dev->d_rdndx)             /* Buffer is empty */
    {
        ...
        */
        ...
        if (ret < 0 || (ret = sem_wait(&dev->d_bfsem)) < 0) {
            ...
            /* The sem_wait() call should fail if we are awakened by a
        ...
        if ((filep->f_oflags & O_RDOK) != 0) {
            ...
            if (--dev->d_nreaders <= 0) {
                ...
                if (PIPE_IS_POLICY_0(dev->d_flags)) {
                    ...
                    while (sem_getvalue(&dev->d_wrsem, &sval) == 0
                            && sval <= 0)
                    {
                        ...
                        size_t len)
{
    ...
    * is because it calls sem_wait() and sem_wait() cannot be called from
        ...
        */
        ...
        if (nxtwrndx != dev->d_rdndx) {
            ...
            */
            ...
            ret = sem_wait(&dev->d_wrsem);
            if (ret < 0 || (ret = sem_wait(&dev->d_bfsem)) < 0) {

                /* Either call sem_wait may fail because a signal was
...
int pipecommon_poll(struct file *filep, struct pollfd *fds,
                    bool setup)
{
    struct inode      *inode = filep->f_inode;
    struct pipe_dev_s *dev   = inode->i_private;
...
int pipecommon_unlink(struct inode *inode)
{
    ...
    dev = (struct pipe_dev_s *)inode->i_private;
