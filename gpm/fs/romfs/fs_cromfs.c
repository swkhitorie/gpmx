...
{
    const struct cromfs_node_s *ff_node;  /* The open file node */
    uint32_t ff_offset;                   /* Cached block offset (zero means none) */
    uint16_t ff_ulen;                     /* Length of decompressed data in cache */
    uint8_t *ff_buffer;                   /* Cached, decompressed data */
};

/* This is the form of the callback from cromfs_foreach_node(): */

typedef int (*cromfs_foreach_t)(const struct cromfs_volume_s *fs,
                                const struct cromfs_node_s *node,
...
{
    struct cromfs_nodeinfo_s *info;   /* Location to return the node info */
    const char *relpath;              /* Full relative path */
    const char *segment;              /* Reference to start of the
                                        * path segment. */
...
static void *cromfs_offset2addr(const struct cromfs_volume_s *fs,
                uint32_t offset);
static uint32_t cromfs_addr2offset(const struct cromfs_volume_s *fs,
                const void *addr);
static int      cromfs_follow_link(const struct cromfs_volume_s *fs,
                const struct cromfs_node_s **ppnode, bool follow,
                struct cromfs_node_s *newnode);
static int      cromfs_foreach_node(const struct cromfs_volume_s *fs,
                const struct cromfs_node_s *node,
                bool follow, cromfs_foreach_t callback, void *arg);
static uint16_t cromfs_seglen(const char *relpath);
static int      cromfs_child_node(const struct cromfs_volume_s *fs,
                const struct cromfs_node_s *node,
                struct cromfs_nodeinfo_s *info);
static int      cromfs_compare_node(const struct cromfs_volume_s *fs,
                const struct cromfs_node_s *node, uint32_t offset,
                void *arg);
static int      cromfs_find_node(const struct cromfs_volume_s *fs,
                const char *relpath,
                struct cromfs_nodeinfo_s *info,
                uint32_t *offset);
...
static int      cromfs_open(struct file *filep, const char *relpath,
                int oflags, mode_t mode);
static int      cromfs_close(struct file *filep);
static ssize_t  cromfs_read(struct file *filep,
                char *buffer, size_t buflen);
static int      cromfs_ioctl(struct file *filep,
                int cmd, unsigned long arg);

static int      cromfs_dup(const struct file *oldp,
                struct file *newp);
static int      cromfs_fstat(const struct file *filep,
                struct stat *buf);

static int      cromfs_opendir(struct inode *mountpt,
                const char *relpath, struct fs_dirent_s **dir);
static int      cromfs_closedir(struct inode *mountpt,
                struct fs_dirent_s *dir);
static int      cromfs_readdir(struct inode *mountpt,
                struct fs_dirent_s *dir,
                struct dirent *entry);
static int      cromfs_rewinddir(struct inode *mountpt,
                struct fs_dirent_s *dir);

static int      cromfs_bind(struct inode *blkdriver,
                const void *data, void **handle);
static int      cromfs_unbind(void *handle, struct inode **blkdriver,
                unsigned int flags);
static int      cromfs_statfs(struct inode *mountpt,
                struct statfs *buf);

static int      cromfs_stat(struct inode *mountpt,
...
static int cromfs_follow_link(const struct cromfs_volume_s *fs,
                            const struct cromfs_node_s **ppnode,
                            bool follow,
                            struct cromfs_node_s *newnode)
{
    const struct cromfs_node_s *linknode;
    const struct cromfs_node_s *node = *ppnode;
    const char *name;
    int i;
    ...
    for (i = 0; i < CROMFS_MAX_LINKS; i++) {
        ...
        linknode = (const struct cromfs_node_s *)
        ...
        */
        ...
        name = (const char *)cromfs_offset2addr(fs, node->cn_name);
...
static int cromfs_foreach_node(const struct cromfs_volume_s *fs,
                            ...
                            void *arg)
{
    const struct cromfs_node_s *pnode;
    struct cromfs_node_s newnode;
    uint32_t offset;
    int ret = 0;
    ...
    */
    ...
    while (pnode != NULL) {
        ...
        pnode  = (const struct cromfs_node_s *)
...
{
    const struct cromfs_node_s *pnode;
    const struct cromfs_node_s *child;
    struct cromfs_node_s newnode;
    uint32_t offset;
    int ret;
    ...
    child  = (const struct cromfs_node_s *)cromfs_offset2addr(fs, offset);
                               ...
                               uint32_t offset, void *arg)
{
    ...
    cpnode = (struct cromfs_comparenode_s *)arg;

    /* Get the name of the node */

    name   = (char *)cromfs_offset2addr(fs, node->cn_name);
    namlen = strlen(name);
    ...
    */
    ...
    if (strncmp(name, cpnode->segment, namlen) == 0) {
        const char *segment = cpnode->segment;
        ...
        */
        ...
        child   = (const struct cromfs_node_s *)
...
{
    struct cromfs_comparenode_s cpnode;
    const struct cromfs_node_s *root;
    int ret;

    /* Get the root node.  The root is the entry "." which is a hard link. */

    root = (const struct cromfs_node_s *)
...
{
    struct inode *inode;
    const struct cromfs_volume_s *fs;
    struct cromfs_nodeinfo_s info;
    struct cromfs_file_s *ff;
    uint32_t offset;
    int ret;
    ...
    */
    ...
    ff = (struct cromfs_file_s *)kmm_zalloc(sizeof(struct cromfs_file_s));
    if (ff == NULL) {
        return -ENOMEM;
    }
    ...
    ff->ff_buffer = (uint8_t *)kmm_malloc(fs->cv_bsize);
    ...
    ff->ff_node = (const struct cromfs_node_s *)
        cromfs_offset2addr(fs, offset);
...
static ssize_t cromfs_read(struct file *filep, char *buffer,
                        size_t buflen)
{
    struct inode *inode;
    const struct cromfs_volume_s *fs;
    struct cromfs_file_s *ff;
    struct lzf_header_s *currhdr;
    struct lzf_header_s *nexthdr;
    uint8_t *dest;
    const uint8_t *src;
    off_t fpos;
    size_t remaining;
    ...
    */
    ...
    ff = (struct cromfs_file_s *)filep->f_priv;
        ...
        do {
            ...
            if (currhdr->lzf_type == LZF_TYPE0_HDR) {
                ...
                struct lzf_type0_header_s *hdr0 =
                    (struct lzf_type0_header_s *)currhdr;
                ...
                struct lzf_type1_header_s * hdr1 =
                    (struct lzf_type1_header_s *)currhdr;
        ...
        */
        ...
        if (currhdr->lzf_type == LZF_TYPE0_HDR) {
            ...
            */
            ...
            src = (const uint8_t *)currhdr + LZF_TYPE0_HDR_SIZE;
            ...
            */
            ...
            if (filep->f_pos <= blkoffs && ulen <= remaining) {
                ...
                */
                ...
                src     = (const uint8_t *)currhdr + LZF_TYPE1_HDR_SIZE;
...
static int cromfs_ioctl(struct file *filep, int cmd, unsigned long arg)
{
    return -ENOTTY;
}
...
static int cromfs_dup(const struct file *oldp, struct file *newp)
{
    struct cromfs_volume_s *fs;
    struct cromfs_file_s *oldff;
    struct cromfs_file_s *newff;

    /* Recover our private data from the struct file instance */

    fs = (struct cromfs_volume_s *)oldp->f_inode->i_private;
    ...
    */
    ...
    newff->ff_buffer = (uint8_t *)kmm_malloc(fs->cv_bsize);
...
static int cromfs_fstat(const struct file *filep, struct stat *buf)
{
    struct inode *inode;
    struct cromfs_volume_s *fs;
    struct cromfs_file_s *ff;
                          ...
                          struct dirent *entry)
{
    ...
    */
    ...
    node = (const struct cromfs_node_s *)cromfs_offset2addr(fs, offset);
    if (node == NULL) {
...
static int cromfs_bind(struct inode *blkdriver, const void *data,
                      void **handle)
{
    /* Return the new file system handle */

    *handle = (void *)&g_cromfs_image;
    return 0;
}
...
static int cromfs_stat(struct inode *mountpt, const char *relpath,
                       struct stat *buf)
{
    const struct cromfs_volume_s *fs;
    struct cromfs_nodeinfo_s info;
