...
static inline int poll_setup(struct pollfd *fds, nfds_t nfds,
                             sem_t *sem)
{
    unsigned int i;
    unsigned int j;
...
static inline int poll_teardown(struct pollfd *fds, nfds_t nfds,
                                int *count, int ret)
{
    unsigned int i;
    int status = 0;
    ...
    */
    ...
    if (ret >= 0) {
        ...
        if (timeout == 0) {
...
#if (((MSEC_PER_TICK) * (USEC_PER_MSEC)) != USEC_PER_TICK) && defined(CONFIG_SYSTEM_TIME64)
            ...
            * already been incremented), sem_tickwait() will not wait, but
            * will return immediately.
            */

            ret = sem_tickwait(&sem, ticks);
            if (ret < 0) {
                if (ret == -ETIMEDOUT) {
