...
int pthread_rwlock_destroy(pthread_rwlock_t *rwlock)
{
    ...
    if (rwlock->rw_refcount != 0 ||
        ...
        for (int i = 0; i < rwlock->rw_condreaders.wait_count; i++) {
            if (rwlock->rw_condreaders.wait_task[i] != NULL) {
                pthread_mutex_unlock(&rwlock->rw_mutex);
                return EBUSY;
            }
        }

        for (int i = 0; i < rwlock->rw_condwriters.wait_count; i++) {
            if (rwlock->rw_condwriters.wait_task[i] != NULL) {
                pthread_mutex_unlock(&rwlock->rw_mutex);
                return EBUSY;
            }
        }
