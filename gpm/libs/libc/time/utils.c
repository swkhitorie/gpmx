...
#include <FreeRTOS.h>
#include <task.h>
#define tick_type_t TickType_t
#elif defined(CONFIG_RTTNANO_ENABLE)
#include <rtdef.h>
#include <rtthread.h>
#define tick_type_t rt_tick_t
#endif
...
int utils_timespec_toticks(const struct timespec *x, void *res)
{
    tick_type_t *res_l = res;
    int ret = 0;
    int64_t total_tick = 0;
    long nano_sec = 0;

    if ( x == NULL || res_l == NULL) {
        ret = EINVAL;
    ...
    if (ret == 0 ) {
...
#elif defined(CONFIG_RTTNANO_ENABLE)
        total_tick = (int64_t)RT_TICK_PER_SECOND * x->tv_sec;
#else
        total_tick = (int64_t)1000 * x->tv_sec;
#endif
        ...
        if (total_tick < 0) {
            ...
            uint32_t ulTickTypeSize = sizeof(tick_type_t);
        ...
        *res_l = (tick_type_t)total_tick;
    }
    return ret;
}

int utils_timespec_todeltaticks(const struct timespec *ab, const struct timespec *cur, void *res)
{
    int ret = 0;
