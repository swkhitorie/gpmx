...
struct cmdarg_s {
    struct nsh_vtbl_s *vtbl;      /* For front-end interaction */
    int fd;                       /* FD for output redirection */
    int argc;                     /* Number of arguments in argv */
    char *argv[MAX_ARGV_ENTRIES]; /* Argument list */
};
#endif
...
struct nsh_memlist_s {
    ...
    char *allocations[CONFIG_NSH_MAXALLOCS];
};
#endif
...
#ifdef HAVE_MEMLIST
static void nsh_memlist_add(struct nsh_memlist_s *memlist,
              char *allocation);
static void nsh_memlist_free(struct nsh_memlist_s *memlist);
#endif

#ifndef CONFIG_NSH_DISABLEBG
static void nsh_releaseargs(struct cmdarg_s *arg);
static void * nsh_child(void * arg);
static struct cmdarg_s *nsh_cloneargs(struct nsh_vtbl_s *vtbl,
               int fd, int argc, char *argv[]);
#endif

static int nsh_saveresult(struct nsh_vtbl_s *vtbl, bool result);
static int nsh_execute(struct nsh_vtbl_s *vtbl,
                int argc, char *argv[], const char *redirfile,
                int oflags);

#ifdef CONFIG_NSH_CMDPARMS
static char *nsh_filecat(struct nsh_vtbl_s *vtbl, char *s1,
                const char *filename);
static char *nsh_cmdparm(struct nsh_vtbl_s *vtbl, char *cmdline,
                char **allocation);
#endif

#ifdef CONFIG_NSH_ARGCAT
static char *nsh_strcat(struct nsh_vtbl_s *vtbl, char *s1,
                const char *s2);
#endif

#if defined(CONFIG_NSH_QUOTE) && defined(CONFIG_NSH_ARGCAT)
static char *nsh_strchr(const char *str, int ch);
#else
#define nsh_strchr(s,c) strchr(s,c)
#endif

#ifdef NSH_HAVE_VARS
static char *nsh_envexpand(struct nsh_vtbl_s *vtbl,
                char *varname);
#endif

#if defined(CONFIG_NSH_QUOTE) && defined(CONFIG_NSH_ARGCAT)
static void nsh_dequote(char *cmdline);
#else
#define nsh_dequote(c)
#endif

static char *nsh_argexpand(struct nsh_vtbl_s *vtbl,
                char *cmdline, char **allocation, int *isenvvar);
static char *nsh_argument(struct nsh_vtbl_s *vtbl, char **saveptr,
                NSH_MEMLIST_TYPE *memlist, int *isenvvar);

#ifndef CONFIG_NSH_DISABLESCRIPT
#ifndef CONFIG_NSH_DISABLE_LOOPS
static bool nsh_loop_enabled(struct nsh_vtbl_s *vtbl);
#endif
#ifndef CONFIG_NSH_DISABLE_ITEF
static bool nsh_itef_enabled(struct nsh_vtbl_s *vtbl);
#endif
static bool nsh_cmdenabled(struct nsh_vtbl_s *vtbl);
#ifndef CONFIG_NSH_DISABLE_LOOPS
static int nsh_loop(struct nsh_vtbl_s *vtbl, char **ppcmd,
                    char **saveptr, NSH_MEMLIST_TYPE *memlist);
#endif
#ifndef CONFIG_NSH_DISABLE_ITEF
static int nsh_itef(struct nsh_vtbl_s *vtbl, char **ppcmd,
                    char **saveptr, NSH_MEMLIST_TYPE *memlist);
#endif
#endif

#ifndef CONFIG_NSH_DISABLEBG
static int nsh_nice(struct nsh_vtbl_s *vtbl, char **ppcmd,
                char **saveptr, NSH_MEMLIST_TYPE *memlist);
#endif

#ifdef CONFIG_NSH_CMDPARMS
static int nsh_parse_cmdparm(struct nsh_vtbl_s *vtbl, char *cmdline,
                const char *redirfile);
#endif

static int nsh_parse_command(struct nsh_vtbl_s *vtbl, char *cmdline);
...
static void nsh_releaseargs(struct cmdarg_s *arg)
{
    struct nsh_vtbl_s *vtbl = arg->vtbl;
    int i;
...
static struct cmdarg_s *nsh_cloneargs(struct nsh_vtbl_s *vtbl,
                                      int fd, int argc, char *argv[])
{
    struct cmdarg_s *ret = (struct cmdarg_s *)malloc(sizeof(struct cmdarg_s));
    memset(ret, 0, sizeof(struct cmdarg_s));
...
static int nsh_saveresult(struct nsh_vtbl_s *vtbl, bool result)
{
    ...
    */
    ...
    if (np->np_lpstate[np->np_lpndx].lp_state == NSH_LOOP_WHILE) {
        np->np_fail = false;
        np->np_lpstate[np->np_lpndx].lp_enable = (result == 0);
        return 0;
    }
                       ...
                       const char *redirfile, int oflags)
{
    ...
    */
    ...
    ret = nsh_builtin(vtbl, argv[0], argv);
    ...
    if (ret >= 0) {
        ...
        */
        ...
        return nsh_saveresult(vtbl, ret != 0);
    }
    ...
    */
    ...
    if (vtbl->np.np_bg) {
        ...
        */
        ...
        ret = pthread_create(&thread, &attr, nsh_child, (void *)args);
        if (ret != 0) {
                             ...
                             const char *filename)
{
    ...
    argument = (char *)realloc(s1, allocsize);
    if (!argument) {
...
static char *nsh_argexpand(struct nsh_vtbl_s *vtbl,
                               char *cmdline, char **allocation,
                               int *isenvvar)
{
    char *working = cmdline;
...
static char *nsh_argexpand(struct nsh_vtbl_s *vtbl,
                               ...
                               int *isenvvar)
{
    char *argument = (char *)g_nullstring;
#ifdef CONFIG_NSH_QUOTE
    char ch = *cmdline;
    ...
    */
    ...
    if (*cmdline == '`') {
        ...
        char *rptr = nsh_strchr(cmdline + 1, '`');
        if (!rptr || rptr[1] != '\0') {
            nsh_error(vtbl, g_fmtnomatching, "`", "`");
            return (char *)g_nullstring;
        }
...
static char *nsh_argument(struct nsh_vtbl_s *vtbl,
                              char **saveptr,
                              ...
                              int *isenvvar)
{
    ...
    */
    ...
    if (*pbegin == '>') {
        ...
        if (*(pbegin + 1) == '>') {
            *saveptr = pbegin + 2;
            argument = (char *)g_redirect2;
        } else {
            *saveptr = pbegin + 1;
            argument = (char *)g_redirect1;
        }
    }
        ...
        if (strcmp(cmd, "nice") == 0) {
            ...
            */
            ...
            if (cmd && strcmp(cmd, "-d") == 0) {
                char *val = nsh_argument(vtbl, saveptr, memlist, NULL);
                if (val) {
