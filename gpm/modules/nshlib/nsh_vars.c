 ...
 *
 ****************************************************************************/
char *nsh_findvar(struct console_stdio_s *pstate, const char *name)
{
    char *ptr;
    char *end;
 ...
 *
 ****************************************************************************/
int nsh_removevar(struct console_stdio_s *pstate, char *pair)
{
    char *end;                 /* Pointer to the end+1 of the NSH variable buffer */
    ...
    if (pair >= pstate->varp && pair < end) {
        ...
        char *src  = &pair[len];       /* Address of name=value string after */
        char *dest = pair;             /* Location to move the next string */
...
int nsh_setvar(struct nsh_vtbl_s *vtbl, const char *name, const char *value)
{
    ...
    */
    ...
    if (pstate->varp != NULL) {
        ...
        newvarp = (char *)realloc(pstate->varp, newsize);
        if (newvarp == NULL) {
            return -ENOMEM;
        }
...
#if !defined(CONFIG_NSH_DISABLE_UNSET)
...
#ifndef CONFIG_NSH_DISABLE_SET
int nsh_foreach_var(struct nsh_vtbl_s *vtbl, nsh_foreach_var_t cb, void *arg)
{
    struct console_stdio_s *pstate = (struct console_stdio_s *)vtbl;
    char *ptr;
    char *end;
