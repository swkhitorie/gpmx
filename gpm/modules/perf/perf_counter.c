...
perf_alloc(enum perf_counter_type type, const char *name)
{
	...
	case PC_COUNT:
        {
			struct perf_ctr_count *obj = kmm_malloc(sizeof(struct perf_ctr_count));
			if (obj) {
				ctr = &obj->head;
			}
		}
	...
	case PC_ELAPSED:
        {
			struct perf_ctr_elapsed *obj = kmm_malloc(sizeof(struct perf_ctr_elapsed));
			if (obj) {
				ctr = &obj->head;
			}
		}
	...
	case PC_INTERVAL:
        {
			struct perf_ctr_interval *obj = kmm_malloc(sizeof(struct perf_ctr_interval));
			if (obj) {
				ctr = &obj->head;
			}
        }
...
{
	struct perf_ctr_count *handle_count = NULL;
	struct perf_ctr_elapsed *handle_elapsed = NULL;
	struct perf_ctr_interval *handle_interval = NULL;
...
perf_print_counter(perf_counter_t handle)
{
	...
	case PC_ELAPSED: {
			...
			KMINFO("%s: %" PRIu64 " events, %" PRIu64 "us elapsed, %.2fus avg, min %" PRIu32 "us max %" PRIu32
                    "us %5.3fus rms\n",
                    handle->name,
                    pce->event_count,
                    pce->time_total,
	...
	case PC_INTERVAL: {
			...
			KMINFO("%s: %" PRIu64 " events, %.2fus avg, min %" PRIu32 "us max %" PRIu32 "us %5.3fus rms\n",
                    handle->name,
                    pci->event_count,
