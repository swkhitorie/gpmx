...
#include "rdline/vt100.h"
#include "rdline/ascii.h"
...
#if defined(CONFIG_READLINE_TABCOMPLETION) && defined(CONFIG_MODULE_BUILTIN)
static int count_builtin_matches(char *buf, int *matches, int namelen)
{
...
#ifdef CONFIG_READLINE_TABCOMPLETION
static void tab_completion(struct rl_common_s *vtbl, char *buf,
                           int buflen, int *nch)
{
    const char *name = NULL;
    char tmp_name[NAME_MAX + 1];

#ifdef CONFIG_MODULE_BUILTIN
...
{
    const char *ret = g_readline_prompt;
