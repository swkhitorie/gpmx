...
static int readline_getc(struct rl_common_s *vtbl)
{
    struct readline_s *priv = (struct readline_s *)vtbl;
    char buffer;
    ssize_t nread;
...
#ifdef CONFIG_READLINE_ECHO
static void readline_putc(struct rl_common_s *vtbl, int ch)
{
    struct readline_s *priv = (struct readline_s *)vtbl;
    char buffer = ch;
    ssize_t nwritten;
...
#ifdef CONFIG_READLINE_ECHO
static void readline_write(struct rl_common_s *vtbl,
                           const char *buffer, size_t buflen)
{
    struct readline_s *priv = (struct readline_s *)vtbl;

    int ret = write(priv->outfd, buffer, buflen);
}
#endif
