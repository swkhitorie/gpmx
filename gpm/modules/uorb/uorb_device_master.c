                ...
                size_t *max_topic_name_length, char **topic_filter, int num_filters)
{
    ...
    *num_topics = 0;
    ...
    for (iterator = master->_node_list.head; iterator != NULL; iterator = iterator->next) {
		...
		if (last_node) {
			last_node->next = kmm_malloc(sizeof(struct __device_node_static_data));
			...
			*first_node = last_node = kmm_malloc(sizeof(struct __device_node_static_data));
			last_node->next = NULL;
...
int uorb_device_master_advertise(struct __uorb_device_master *master, const struct orb_metadata *meta, bool is_advertiser, int *instance, enum ORB_PRIO priority)
{
	...
	do {
		...
		int nodepath_len = strlen(nodepath) + 1;
		char *devpath = (char *)kmm_malloc(nodepath_len);
		if (devpath != NULL) {
			memcpy(devpath, nodepath, nodepath_len);
		}
		const char *devpath_c = devpath;
        ...
        struct __uorb_device_node *node = kmm_malloc(sizeof(struct __uorb_device_node));
        uorb_device_node_init(node, meta, group_tries, devpath_c, priority, 1);
