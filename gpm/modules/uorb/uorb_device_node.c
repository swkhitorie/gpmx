            ...
            const char *path, enum ORB_PRIO priority, uint8_t queue_size)
{
	...
	unode->_data = NULL;
	unode->_subscriber_count = 0;
	unode->_advertised = NULL;
...
int uorb_device_node_open(struct urbnode *node)
{
	...
	if (unode->nd._oflags == UORB_F_RDONLY) {
		...
		struct __subscriber_data *sd = kmm_malloc(sizeof(struct __subscriber_data));
...
ssize_t uorb_device_node_write(struct urbnode *node, const char *buffer, size_t buflen)
{
	...
	if (NULL == unode->_data) {
		...
		if (!up_interrupt_context()) {
			...
			/* re-check size */
			if (NULL == unode->_data) {
				unode->_data = kmm_malloc(unode->_meta->o_size * unode->_queue_size);
			}
	...
	case ORBIOCSETINTERVAL: {
			...
			if (arg == 0) {
				...
				if (sd->update_interval) {
					...
					sd->update_interval = kmm_malloc(sizeof(struct __update_interval_data));
