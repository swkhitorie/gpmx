...
int register_uorb_driver(const char *name, void *data)
{
    int ret = -ENOSPC;
...
int unregister_uorb_driver(const char *name)
{
    int ret = -EINVAL;
    if (name == NULL) {
        return -EINVAL;
...
{
    if (node->_registered) {
        unregister_uorb_driver(node->_devname);
    }
...
int uorbnode_register(struct urbnode *node)
{
	...
	if (node->_devname != NULL) {
		ret = register_uorb_driver(node->_devname, (void *)node);
