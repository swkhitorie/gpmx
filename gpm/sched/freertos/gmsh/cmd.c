...
int list_rtos(int argc, char **argv)
{
    ...
    for (i = 0; i < sz; i++) {
        ...
        gsh_kprintf("[task] %15s %c, pri:%2lu, bptr=%x, wptr=%x, size=%6lu, used=%6lu, %5.1f%%, cpu:%.2f%%\r\n",
            status->pcTaskName,
...
int list_queue(int argc, char **argv)
{
    ...
    for (int i = 0; i < configQUEUE_REGISTRY_SIZE; i++) {
        ...
        gsh_kprintf("[queue] %-18s,ptr:%p, curMsgs:%03u, maxUsedMsgs:%03u, capacity:%03u, item_size:%03u\r\n",
...
int systime_handle(int argc, char **argv)
{
    if (argc > 2 && !strcmp(argv[1], "set")) {
        ...
        gsh_kprintf("[system_time] Uptime (since boot): %u.%03u s, HRT: %.3f s\r\n", boot_time.tv_sec, boot_time.tv_nsec/1000000, hrt_absolute_time()/1e6f);
    } else {
        gsh_kprintf("Usage: system_time <set/get>\r\n");
    }
    return 0;
}
