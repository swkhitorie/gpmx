...
void mutex_t1(void *p)
{
    ...
    for (;;) {
        ...
        KMRAW("mutex_t1: low take mutex blocking \n");
        int ret = xSemaphoreTake(mutex, portMAX_DELAY);
        KMRAW("mutex_t1: low mutex taked %d \n", ret);
...
void mutex_mid(void *p)
{
    ...
    for (;;) {
        cnt++;
        KMRAW("mutex_mid task take sem blocking \n");
...
void mutex_t2(void *p)
{
    int cnt = 1;
    for (;;) {
        KMRAW("mutex_t2: high take mutex blocking \n");
        int ret = xSemaphoreTake(mutex, portMAX_DELAY);
        KMRAW("mutex_t2: high mutex taked %d \n", ret);
    
        KMRAW("mutex_t2: high give mutex \n");
        ret = xSemaphoreGive(mutex);
        vTaskDelay(1000);
    }
}
