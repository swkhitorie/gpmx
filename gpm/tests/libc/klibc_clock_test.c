...
void* clk_entry(void *p)
{
    ...
    for (;;) {
        ...
        clock_gettime(CLOCK_REALTIME, &x1);
        KMRAW("[%s] clock gettime: %d %d\r\n", &name[0], x1.tv_sec, x1.tv_nsec);
...
int klibc_clock_test(int argc, char **argv)
{
    ...
    pthread_attr_setstacksize(&clk_p.attr, 512*4);
