...
MK_APPENDLINE=$(if $(and ${1},${2}),@${MK_BUSYBOX} sh -c 'for i in ${1}; do echo $$i >> ${2}; done')
MK_APPGET=$(if $(and ${1},${2}),@${MK_BUSYBOX} ${1}=$(car ${2}))
else
MK_APPEND=$(if $(and ${1},${2}),@echo ${1}>>${2})
MK_APPENDLINE=$(if $(and ${1},${2}),@for i in ${1}; do echo $$i >> ${2}; done)
define MK_APPENDCMACROSFILE
	$(if $(and $(1),$(2),$(3)),
	$(eval MK_GUARD:=$(3))
	$(call MK_APPEND,"/* Auto-generated by MK_APPENDCMACROSFILE */",$(2))
	$(call MK_APPEND,"#ifndef $(MK_GUARD)",$(2))
	$(call MK_APPEND,"#define $(MK_GUARD)",$(2))
	$(call MK_APPEND,"",$(2))
	$(foreach def,$(1),
		$(if $(findstring =,$(def)),
			$(eval MK_TMP_KEY := $(firstword $(subst =, ,$(def))))
			$(eval MK_TMP_VAL := $(subst $(MK_TMP_KEY)=,,$(def)))
			$(if $(filter n,$(MK_TMP_VAL)),,
				$(if $(filter y,$(MK_TMP_VAL)),
					$(call MK_APPEND,'#define $(MK_TMP_KEY)',$(2)),
					$(call MK_APPEND,'#define $(MK_TMP_KEY) $(MK_TMP_VAL)',$(2))
				)
			),
			$(call MK_APPEND,'#define $(def)',$(2))
		)
	)
	$(call MK_APPEND,"",$(2))
	$(call MK_APPEND,"#endif /* $(MK_GUARD) */",$(2))
	)
endef
