#ifndef DEV_OPS_SDIO_H_
#define DEV_OPS_SDIO_H_
...
struct sdio_dev_s
{
  ...
  int   (*lock)(struct sdio_dev_s *dev, bool lock);
#endif

  /* Initialization/setup */

  void  (*reset)(struct sdio_dev_s *dev);
  sdio_capset_t (*capabilities)(struct sdio_dev_s *dev);
  sdio_statset_t (*status)(struct sdio_dev_s *dev);
  void  (*widebus)(struct sdio_dev_s *dev, bool enable);
  void  (*clock)(struct sdio_dev_s *dev, enum sdio_clock_e rate);
  int   (*attach)(struct sdio_dev_s *dev);

  /* Command/Status/Data Transfer */

  int   (*sendcmd)(struct sdio_dev_s *dev, uint32_t cmd,
                        uint32_t arg);
#ifdef CONFIG_SDIO_BLOCKSETUP
  void  (*blocksetup)(struct sdio_dev_s *dev, unsigned int blocklen,
                    unsigned int nblocks);
#endif
  int   (*recvsetup)(struct sdio_dev_s *dev, uint8_t *buffer,
                    size_t nbytes);
  int   (*sendsetup)(struct sdio_dev_s *dev,
                        const uint8_t *buffer, size_t nbytes);
  int   (*cancel)(struct sdio_dev_s *dev);

  int   (*waitresponse)(struct sdio_dev_s *dev, uint32_t cmd);
  int   (*recv_r1)(struct sdio_dev_s *dev, uint32_t cmd,
                    uint32_t *R1);
  int   (*recv_r2)(struct sdio_dev_s *dev, uint32_t cmd,
                    uint32_t R2[4]);
  int   (*recv_r3)(struct sdio_dev_s *dev, uint32_t cmd,
                    uint32_t *R3);
  int   (*recv_r4)(struct sdio_dev_s *dev, uint32_t cmd,
                    uint32_t *R4);
  int   (*recv_r5)(struct sdio_dev_s *dev, uint32_t cmd,
                    uint32_t *R5);
  int   (*recv_r6)(struct sdio_dev_s *dev, uint32_t cmd,
                    uint32_t *R6);
  int   (*recv_r7)(struct sdio_dev_s *dev, uint32_t cmd,
                    uint32_t *R7);

  /* Event/Callback support */

  void  (*waitenable)(struct sdio_dev_s *dev,
                    sdio_eventset_t eventset, uint32_t timeout);
  sdio_eventset_t (*eventwait)(struct sdio_dev_s *dev);
  void  (*callbackenable)(struct sdio_dev_s *dev,
  ...
  int   (*dmapreflight)(struct sdio_dev_s *dev,
                        const uint8_t *buffer, size_t buflen);
#endif
  int   (*dmarecvsetup)(struct sdio_dev_s *dev, uint8_t *buffer,
                        size_t buflen);
  int   (*dmasendsetup)(struct sdio_dev_s *dev,
                        const uint8_t *buffer, size_t buflen);
#endif

  void  (*gotextcsd)(struct sdio_dev_s *dev,
...
int sdio_probe(struct sdio_dev_s *dev);

int sdio_set_wide_bus(struct sdio_dev_s *dev);

int sdio_set_blocksize(struct sdio_dev_s *dev, uint8_t function,
                        uint16_t blocksize);

int sdio_enable_function(struct sdio_dev_s *dev, uint8_t function);

int sdio_enable_interrupt(struct sdio_dev_s *dev, uint8_t function);

int sdio_io_rw_direct(struct sdio_dev_s *dev, bool write,
                    uint8_t function, uint32_t address,
                    uint8_t inb, uint8_t *outb);

int sdio_io_rw_extended(struct sdio_dev_s *dev, bool write,
                        uint8_t function, uint32_t address,
