...
uint8_t MPU6050::register_read(Register reg)
{
    uint8_t cmd[2] {};
    ...
    rmsg.rbuffer = &cmd[1];
    rmsg.rlength = 1;
    ret = I2C_TRANSFER(ops, &rmsg, 1);

    return cmd[1];
}

void MPU6050::register_write(Register reg, uint8_t value)
{
    uint8_t cmd[2] {};
    cmd[0] = static_cast<uint8_t>(reg);
    cmd[1] = value;
...
void MPU6050::register_modify(Register reg, uint8_t setbits, uint8_t clearbits)
{
