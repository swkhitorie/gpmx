...
/**
 * GeodesicGrid is a class for working on geodesic sections.
 *
...
/**
    改动-> 
    _inverses[10] 和 _mid_inverses[10] 的隐式转换被explicit禁止, 遂间接声明float[3*3]来初始化
    vector3f 和 matrix3f 的本地转换
    is_zero() 和 is_equal() 的本地转换
    auto 无法识别 matrix3f * vector3f 的结果为 vector3f, 遂直接声明w 为vector3f
    .x .y .z 转换为(0), (1), (2)
*/
class GeodesicGrid {
...
public:
     ...
     * inclusive is false.
     */
    static int section(const matrix::Vector3f &v, bool inclusive = false);
...
private:
     ...
     */
    static const matrix::Matrix3f _inverses[10];
     ...
     */
    static const matrix::Matrix3f _mid_inverses[10];
     ...
     */
    static int _triangle_index(const matrix::Vector3f &v, bool inclusive);
