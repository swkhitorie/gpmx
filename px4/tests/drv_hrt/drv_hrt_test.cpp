...
void hrt_call1(void *arg)
{
    ...
    TEST_PRINTF("%.6f call 1 run | iscalled?:%d | arg: %d | in irq?:%d \r\n", now,
...
{
    float now = hrt_absolute_time()/1e6f;
    TEST_PRINTF("%.6f call 2 run | iscalled?:%d | arg: %.5f \r\n", now, hrt_called(&call2), *(float *)arg);
}
...
{
    float now = hrt_absolute_time()/1e6f;
    TEST_PRINTF("%.6f call 3 run | iscalled?:%d | arg: %.5f \r\n", now, hrt_called(&call3), *(double *)arg);
}

void hrt_test(void *p)
{
    // should call hrt_init() in init process;
