...
{
    lld_gpio_init(&obj->it_pin, port, pin, mode, GPIO_Speed_50MHz, true);
    obj->it_line = (uint32_t)(0x01 << pin);
    
    uint8_t portsource;
	if     (port==GPIOA)	portsource = 0; 
	else if(port==GPIOB) 	portsource = 1; 
	else if(port==GPIOC)	portsource = 2; 
	else if(port==GPIOD)	portsource = 3;	
	else if(port==GPIOE)	portsource = 4;	
	else if(port==GPIOF)	portsource = 5;	
	else if(port==GPIOG)	portsource = 6;
    GPIO_EXTILineConfig(portsource, obj->it_line);
