...
static void lld_i2c_io(lld_i2c_t *obj, GPIOMode_TypeDef mode, bool af, bool remap);
static bool lld_i2c_reset(lld_i2c_t *obj);

void lld_i2c_io(lld_i2c_t *obj, GPIOMode_TypeDef mode, bool af, bool remap)
{
    if (remap && obj->i2cnum == 1) {
        if (mode == GPIO_Mode_AF_OD) {
            GPIO_PinRemapConfig(GPIO_Remap_I2C1, ENABLE);
        }
        lld_gpio_init(&obj->scl, GPIOB, 8, mode, GPIO_Speed_50MHz, true);
        lld_gpio_init(&obj->sda, GPIOB, 9, mode, GPIO_Speed_50MHz, true);
        
    } else {
        //			                I2C1	I2C2
        GPIO_TypeDef *sclport[2] = {GPIOB,   GPIOB};
        uint16_t      sclpin[2]  = {6,       10   };
        GPIO_TypeDef *sdaport[2] = {GPIOB,   GPIOB};
        uint16_t      sdapin[2]  = {7,       11   };
        
        lld_gpio_init(&obj->scl, sclport[obj->i2cnum - 1], sclpin[obj->i2cnum - 1], mode, GPIO_Speed_50MHz, true);
        lld_gpio_init(&obj->sda, sdaport[obj->i2cnum - 1], sdapin[obj->i2cnum - 1], mode, GPIO_Speed_50MHz, true);
    }
}
...
bool lld_i2c_reset(lld_i2c_t *obj)
{
	...
	while (I2C_GetFlagStatus(obj->i2c, I2C_FLAG_BUSY) && cnt < 20) {
        ...
        lld_i2c_io(obj, GPIO_Mode_Out_PP, false, obj->remap); //Initialize GPIO for Software handle
		...
		for (volatile uint16_t i = 0; i < 5; i++);
        lld_i2c_io(obj, GPIO_Mode_IN_FLOATING, false, obj->remap);
		RCC->APB1RSTR |= obj->rcc;      //Start reset I2C Clock
		RCC->APB1RSTR &= ~obj->rcc;     //Stop  reset I2C Clock
		RCC->APB1ENR |=  obj->rcc;      //Enable I2C clock
...
bool lld_i2c_init(lld_i2c_t *obj, uint8_t i2c_num, uint32_t speed, bool remap)
{
    ...
    I2C_TypeDef *i2clist[2] = {I2C1, I2C2};
    uint32_t i2c_rcclist[2] = {RCC_APB1Periph_I2C1, RCC_APB1Periph_I2C2};
    uint32_t i2c_eventirq_list[2] = {I2C1_EV_IRQn, I2C2_EV_IRQn};
    uint32_t i2c_errorirq_list[2] = {I2C1_ER_IRQn, I2C2_ER_IRQn};
    obj->i2c = i2clist[obj->i2cnum - 1];
    obj->rcc = i2c_rcclist[obj->i2cnum - 1];
    
    lld_i2c_io(obj, GPIO_Mode_AF_PP, true, obj->remap);
	if (!lld_i2c_reset(obj)) {
		obj->state = I2C_STATE_ERROR;
		return false;
	}
    lld_i2c_io(obj, GPIO_Mode_AF_OD, true, obj->remap);
