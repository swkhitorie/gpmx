...
void lld_dac_init(lld_dac_t *obj, uint8_t num)
{
	...
	if (num == 1) {
        lld_gpio_init(&obj->pin, GPIOA, 4, GPIO_Mode_IN, GPIO_OType_PP, GPIO_PuPd_NOPULL, GPIO_High_Speed, 0);
        lld_gpio_set(&obj->pin, 1);
	}else if (num == 2) {
        lld_gpio_init(&obj->pin, GPIOA, 6, GPIO_Mode_IN, GPIO_OType_PP, GPIO_PuPd_NOPULL, GPIO_High_Speed, 0);
        lld_gpio_set(&obj->pin, 1);
	} 
