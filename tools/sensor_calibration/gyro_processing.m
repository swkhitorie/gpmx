...
clear;  %清空工作区
%dlmread
data = dlmread('gyro_sample.dat');         %从文本中读取数据，单位：deg/s，速率：100Hz
data = data(1:720000, 3:5)*3600;    %截取两个小时的数据，把 deg/s 转为 deg/h
[A, B] = allan_varance2(data, 100, 100);     %求Allan标准差，用100个点来描述
...
C(1, :) = fitting(sqrt(A'), B(:, 1)', 2)';   %拟合
C(2, :) = fitting(sqrt(A'), B(:, 2)', 2)';
C(3, :) = fitting(sqrt(A'), B(:, 3)', 2)';
